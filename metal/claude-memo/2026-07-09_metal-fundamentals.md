...
- **低レベル・明示的なGPU API**。OpenGLのような「暗黙のグローバルステートマシン」ではなく、
  「何を描画/計算するか」を事前に**パイプラインステートオブジェクト**として固めておき、
  実行時はそれを選んでコマンドを積むだけ、という設計(Vulkan/DirectX12と同系統)。
  ドライバがその場で状態を検証・翻訳するコストを減らすのが狙い。
- **CPUとGPUは非同期**。CPU側は「コマンドを積む(encode)」→「GPUに投げる(commit)」までしかせず、
  基本的にブロックしない。GPUが実際にそれを実行するのは後(必要な時だけ`waitUntilCompleted()`で待つ)。
- **3つの言語バインディングが存在する**: Objective-C(`Metal.h`、ネイティブ)、Swift、
  そして `metal-cpp`(Appleが公式に配布するヘッダオンリーのC++ラッパー。ARCが効かないので
  手動retain/releaseが必要 → 詳細は6章)。このプロジェクトでは主に`metal-cpp`、
...
- **Device**: GPUそのものへのハンドル。ほぼ全てのMetalオブジェクトはDeviceから作る
  (`MTL::CreateSystemDefaultDevice()`)。
- **CommandQueue**: コマンドバッファの投入窓口。基本はアプリ全体で1個作って使い回す。
- **CommandBuffer**: 1回分の「やることリスト」の入れ物。作って、中にEncoderで命令を積んで、`commit()`する。
